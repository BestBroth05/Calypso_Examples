 ...
 *
 * <p>
 * This class demonstrates the various steps involved in PIN code verification
 * for a Calypso
 ...
 *
 * <ul>
 * <li>Identify the presence of an ISO 14443-4 card and activate the card
 * selection manager for
 * targeted operations.
 * <li>Initiate an AID-based application selection scenario to pinpoint and
 * select a specific
 * Calypso card.
 * <li>Set ciphering keys needed for the PIN change operation.
 * <li>Create a Secure Regular Mode Transaction Manager for encrypted PIN change
 * operations.
 * <li>Prompt the user to enter a new 4-digit numeric PIN code and validate the
 * input.
 ...
 * cause loss of
 * communication with the smart card (the card is reset after 5 seconds of
 * inactivity).
 *
 * <p>
 * Operations and outcomes are meticulously logged using slf4j, offering
 * detailed insights for
 * tracking and debugging. In scenarios where unexpected behaviors or errors
 * emerge, runtime
 * exceptions are triggered, offering precise, actionable feedback for timely
 * resolution.
 */
...
public class Main_ChangePin_Pcsc {
  ...
  public static void main(String[] args) throws InterruptedException, IOException {
    ...
    // Instantiate a Secure Regular Mode Transaction Manager to handle encrypted PIN
    // verification
    // and secure operations.
    SecureRegularModeTransactionManager cardTransaction = calypsoCardApiFactory
   ...
   *
   * <p>
   * Gets an instance of the smart card service, registers the PC/SC plugin, and
   * prepares the
   * reader API factory for use.
   *
   * <p>
   * Retrieves the {@link ReaderApiFactory}.
   */
   ...
   *
   * <p>
   * Prepares the card reader using a predefined set of configurations, including
   * the card reader
   ...
   *
   * <p>
   * Prepares the SAM reader using a predefined set of configurations, including
   * the card reader
   * name regex, ISO protocol, and sharing mode.
   */
   ...
   *
   * <p>
   * Prepares the SAM reader, selects the SAM, and sets up the symmetric crypto
   * security setting
   ...
   *
   * <p>
   * Retrieves the {@link CalypsoCardApiFactory}.
   */
   ...
   *
   * <p>
   * It finds the reader name by matching with a regular expression, then
   ...
   *
   * <p>
   * Creates a SAM selection manager, prepares the selection, and processes the
   * SAM selection
   * scenario.
   *
  ...
  private static LegacySam selectSam(CardReader reader) {
    ...
    IsoCardSelector cardSelector = readerApiFactory
        .createIsoCardSelector()
        .filterByPowerOnData(
            LegacySamUtil.buildPowerOnDataFilter(LegacySam.ProductType.SAM_C1, null));

    LegacySamApiFactory legacySamApiFactory = LegacySamExtensionService.getInstance().getLegacySamApiFactory();
    ...
    CardSelectionResult samSelectionResult = samSelectionManager.processCardSelectionScenario(reader);
   ...
   *
   * <p>
   * Creates a card selection manager, prepares the selection using the provided
   * AID, and
   * processes the card selection scenario. The AID is used to identify and select
   * the specific
   ...
   * @throws IllegalStateException if the selection of the application identified
   *                               by the provided
   *                               AID fails.
   */
  private static CalypsoCard selectCard(CardReader reader, String aid) {
    ...
    CalypsoCardSelectionExtension calypsoCardSelectionExtension = calypsoCardApiFactory
        .createCalypsoCardSelectionExtension().acceptInvalidatedCard();
