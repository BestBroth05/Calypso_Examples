...
/**
 * Handles the process of debiting the Stored Value (SV) of a Calypso card using
 * the PC/SC plugin
 * and the Calypso Card Extension Service.
 *
 * <p>
 * This class demonstrates the SV debit process inside a Secure Session. It
 * includes the
 * initialization of the Smart Card Service, registering the PC/SC plugin,
 * checking the
 * compatibility of the Calypso card extension service, and performing
 * operations with the Keypop
 * Reader and Calypso Card APIs.
 *
 * <h2>Key Functionalities</h2>
 *
 * <ul>
 * <li>Initialization of the Smart Card Service and registering the PC/SC
 * plugin.
 ...
 *
 * <p>
 * Each operation is logged for tracking and debugging purposes. In case of an
 * error during the
 * card authentication process, an {@link IllegalStateException} is thrown and
 * logged to the
 * console.
 */
@SuppressWarnings("deprecation")
public class Main_StoredValue_DebitInSession_Pcsc {
  private static final Logger logger = LoggerFactory.getLogger(Main_StoredValue_DebitInSession_Pcsc.class);
  ...
  private static CardReader getReader(
    ...
    if (reader == null) {
      throw new IllegalStateException("No se encontró ningún lector conectado que coincida con: " + readerNameRegex);
    }
